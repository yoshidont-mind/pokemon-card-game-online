...
- [x] ゾーン移動時のハイライト挙動が仕様通り
- [x] カード付与時のハイライト挙動が仕様通り
- [x] DnD 失敗時に状態が破壊されない
...
- [x] カード/ダメカン/状態異常バッヂをドラッグ可能
- [x] ゾーン重なり時に対象ゾーンが赤ハイライト
- [x] カード重なり時に対象カードが赤ハイライト
- [x] ドロップ確定時のみ状態更新（途中更新なし）
- [x] 無効ドロップ/キャンセル時に状態破壊なし
- [x] 手札トレイをハンドル操作で任意位置へ移動できる
- [x] 手札トレイ位置のリセット（初期位置復帰）ができる
- [x] `CI=true npm test -- --watch=false` と `npm run build` が通る
- [x] 実装ログが `references/implementation_logs/260218_phase04_drag_and_drop_foundation.md` に記録される
...
## 13. 追加実装: 手札トレイのドラッグ移動（ユーザー指定の拡張）

### 13.1 目的

- 一般的なPC解像度で手札展開時にベンチと重なる問題を軽減する
- プレイヤー自身が状況に応じて手札トレイ位置を調整できるようにする
- 将来のUI拡張（配置最適化）に再利用可能な基盤を先に実装する

### 13.2 仕様（本フェーズで固定）

- 手札トレイ上部に「移動ハンドル」を配置し、ハンドルを掴んだドラッグでトレイ全体を移動可能にする
- カードDnDとは干渉させない（カード領域は従来どおりカード操作専用）
- トレイは画面外へ出ないようにクランプする（上下左右に最小マージンを確保）
- 「位置リセット」操作で初期位置（中央下）へ戻せるようにする
- 位置はブラウザに保存し、同一端末/同一ブラウザで再表示時に復元する

### 13.3 実装手順

1. `src/components/HandTray.js`
- トレイ位置 state（`x`,`y`）を追加
- Pointer events を用いた移動ロジックを追加
- ハンドルUIとリセットUIを追加
- localStorage 読み書きを追加（失敗時は no-op）

2. `src/css/playingField.module.css`
- ハンドル行・リセットボタンのスタイルを追加
- ドラッグ中の視覚フィードバック（カーソル、陰影）を追加

3. 回帰確認
- 既存 DnD（カード/小道具）と干渉しないこと
- `#hand-tray-panel` 被覆時のドロップブロック挙動が維持されること
...
手動検証項目:
- 手札トレイをドラッグして任意位置へ移動できる
- 画面外へ出ない
- リロード後に位置が保持される
- 位置リセットで中央下へ戻る
- 手札カードDnD/小道具DnDが従来どおり動作する
